"""
Realistic mock market data generator.
...
# Typical spread basis points per asset class
_SPREAD_BPS: dict[str, float] = {
    # Crypto — wider spreads
    "BTC": 2.0, "ETH": 2.5, "SOL": 4.0, "BNB": 3.5, "XRP": 5.0,
    "ADA": 6.0, "AVAX": 5.5, "DOGE": 8.0, "DOT": 7.0, "LINK": 6.5,
    "MATIC": 7.5, "UNI": 8.5, "LTC": 5.0, "ATOM": 7.0, "FIL": 9.0,
    "APT": 8.0, "ARB": 9.0, "OP": 9.5, "INJ": 7.5, "SUI": 10.0,
    # Equities — tighter spreads
    "AAPL": 1.0, "MSFT": 1.0, "NVDA": 1.5, "GOOGL": 1.0, "AMZN": 1.5,
    "TSLA": 2.0, "META": 1.0, "AMD": 1.5, "NFLX": 2.0, "COIN": 3.0,
}
...
def get_quote(symbol: str) -> dict | None:
    """
    Return a full Level-1 quote with bid, ask, spread, open interest,
    and VWAP in addition to the standard price fields.
    """
    symbol = symbol.upper()
    base = _BASE.get(symbol)
    if base is None:
    ...
    spread_bps = _SPREAD_BPS.get(symbol, 5.0)
    half_spread = price * (spread_bps / 10_000) / 2
    bid = price - half_spread
    ask = price + half_spread
    spread = ask - bid

    # Simple VWAP approximation: weighted average of high, low, close
    vwap = (high + low + price) / 3

    # Mock open interest (only meaningful for crypto; use volume proxy for stocks)
    open_interest = volume * random.uniform(0.3, 0.7)

    precision = 6 if price < 1 else 4
    ...
    return {
        ...
        "spread_bps": round(spread_bps + random.uniform(-0.5, 0.5), 2),
        "change_24h": round(change_24h, 4),
        "change_24h_pct": round(change_24h, 2),
        "volume_24h": round(volume, 0),
        "open_interest": round(open_interest, 0),
        "vwap": round(vwap, 6 if price < 1 else 2),
        "market_cap": base.get("market_cap"),
        "high_24h": round(high, 6 if price < 1 else 2),
        "low_24h": round(low, 6 if price < 1 else 2),
        "timestamp": datetime.now(timezone.utc),
